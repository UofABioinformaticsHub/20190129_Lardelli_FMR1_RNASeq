...
Reviewer comments addressed the need to BLAST against features such as introns, promoters and UTR's, opposed to only cDNA sequences. BLASTing against the genome will ensure similar sequences within these features do not go undetected.
...
To get sequences from a GRanges object, Biostrings::getSeq() requires a genome build in FASTA format. I downloaded the Ensembl version 101 zebrafish GRCz11 build from [Ensembl's ftp server](ftp://ftp.ensembl.org/pub/release-101/fasta/danio_rerio/dna/).
...
The reviewer also expressed their interest in knowing "what mRNA fragment length was set to find the similarity, from El Brolosy et al ‘Optimal thresholds varied for the different cell line models and ranged between 1) 20 and 180 nucleotides (alignment length)'". While a mRNA fragment length does not need to be set for a BLAST analysis, El. Brolosy et al performed a sub-sampling analysis to determine what alignment length was optimal for identifying homology. The methods of this study are not clearly detailed, but an attempt will be made to re-create this procedure.
...
```{r commonVals}
genes <- unique(topTable_glm$external_gene_name)
rep <- 10000
...
# 			is_greater_than(0) %>%
# 			sum(na.rm = TRUE)
# 		samples <- lapply(seq(rep), function(y){sample(x = genes, size = nrow(lenTable))})
...
# 			is_greater_than(0) %>%
# 			sum(na.rm = TRUE)
# 		samples <- lapply(seq(rep), function(y){sample(x = genes, size = nrow(bitTable))})
# 		upSamples <- lapply(samples, function(z){
# 			z %>%
# 				tibble("Gene" = .) %>% 
# 				left_join(lfcTable, by = "Gene") %>% 
...
# 			is_greater_than(0) %>%
# 			sum(na.rm = TRUE)
# 		samples <- lapply(seq(rep), function(y){sample(x = genes, size = nrow(eTable))})
# 		upSamples <- lapply(samples, function(z){
# 			z %>%
# 				tibble("Gene" = .) %>% 
# 				left_join(lfcTable, by = "Gene") %>% 
